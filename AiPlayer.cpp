...
void BattleShip::AiPlayer::placeShips() {
  ...
  for(const auto& ship : shipHealths) { //check for overlapping ships
    char orientation = *chooseRandom(orientation_choice, randomNumberGenerator); //printing extra boards in beginning
    do {
      if (orientation == 'h') {
    ...
    std::cout << "AI " << getId()+1 << "'s Board" << std::endl;
